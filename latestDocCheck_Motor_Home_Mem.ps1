   ...
   ############################################################################################################################
    ...
    foreach($sourceItem in $DataSet.Tables[0])
    {
        ...
        {        
            $sourceFolder = $sourcePath+'\'+$product
        }
        else
        {
        ...
        $sourceSearch = $source+'*'
        $fileName = Get-ChildItem -Path $sourceFolder -Recurse | Where-Object {(!$_.PsIsContainer) -and ($_.Name -like $sourceSearch) } | Select-Object -First 1
